    ...
    <section className="mt-7" aria-labelledby="results-title">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-[#53665e]" role="status">{busy ? results ? `${statusLabel} ${t('search.refreshing_results')}` : statusLabel : phase === 'search-error' ? results ? t('search.refreshing_results') : '' : locationFailure && results ? t('search.previous_results') : dirty ? t('search.filters_changed') : results ? t('search.results_summary', { count: doctors.length, radius: results.filters.radius }) : t('search.waiting_location')}</p>
        </div>
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
        <div className={`${mobileView === 'list' ? 'block' : 'hidden'} min-w-0 lg:block`} aria-busy={busy}>
          {busy && !results ? <div className="panel"><LoadingState label={statusLabel} /></div> : !results && phase === 'search-error' ? null : !results ? <div className="panel empty-state"><h3 className="font-bold">{t('search.waiting_location')}</h3><p className="mt-2 text-sm text-[#53665e]">{mode === 'manual' ? t('search.manual_hint') : t('search.refresh_hint')}</p></div> : doctors.length ? <div className="space-y-3">{doctors.map(doctor => <DoctorCard key={doctor.id} doctor={doctor} onClick={setSelectedDoctor} />)}</div> : busy ? <div className="panel"><LoadingState label={statusLabel} /></div> : phase === 'search-error' ? null : <div className="panel empty-state">
